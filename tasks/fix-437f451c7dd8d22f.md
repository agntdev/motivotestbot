# fix-437f451c7dd8d22f — E1T2 /unsubscribe loses user granularity — keys on chatId only

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

E1T1 stores subscribers per `(chatId, userId)` pair (`src/handlers/E1T1.ts:43`: key `${chatId}:${userId}`), correctly scoping subscription to an individual user within a chat. E1T2's `removeSubscriber` (`src/store/subscriptions.ts:37-44`) only keys on `chatId` — it never looks at `ctx.from?.id`. This means even if the storage were unified, `/unsubscribe` would delete a chat-level key unrelated to the per-user subscription keys that `/subscribe` creates.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-437f451c7dd8d22f.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-437f451c7dd8d22f.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-437f451c7dd8d22f.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
