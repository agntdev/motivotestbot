# fix-63967eb51bb018c2 — /subscribe not idempotent — violates docs/spec.md requirement

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

`docs/spec.md` (line 24-26) specifies: "If already subscribed, reply idempotently: 'You're already subscribed.'" Neither `E1T1.ts` nor `fix-ecade9319396d9c9.ts` checks for an existing subscription before writing. Re-subscribing a user silently overwrites the record and sends the same "subscribed successfully" message, violating the idempotency contract.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-63967eb51bb018c2.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-63967eb51bb018c2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-63967eb51bb018c2.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
