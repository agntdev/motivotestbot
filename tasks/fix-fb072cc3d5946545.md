# fix-fb072cc3d5946545 — Duplicate command handlers cause multi-reply per command

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

The auto-loader in `bot.ts:35` imports ALL `.js` handler files. Multiple handlers register the same grammY command, and grammY invokes all of them in sequence. Results: `/subscribe` → 2 replies (E1T1.ts + fix-ecade9319396d9c9.ts); `/unsubscribe` → 3 replies (E1T2.ts + fix-2a2cc0770e5563ca.ts + fix-ecade9319396d9c9.ts); `/quote` → 2 replies (E2T1.ts + E4T1.ts). Users get duplicate/triplicate messages for every command. Additionally, E1T1 and fix-ecade9319396d9c9 write to different subscriber stores, so subscribe/unsubscribe flow is inconsistent.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-fb072cc3d5946545.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-fb072cc3d5946545.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-fb072cc3d5946545.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
