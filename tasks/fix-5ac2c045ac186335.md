# fix-5ac2c045ac186335 — Dual daily cron jobs — duplicate messages sent every morning

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

Both `E3T1.ts` (line 34) and `E3T2.ts` (line 10) schedule `cron.schedule("0 9 * * *", ...)`. Each runs independently at 09:00 UTC, sending two messages to every subscriber daily (one generic "Good morning" and one quote).

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-5ac2c045ac186335.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-5ac2c045ac186335.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-5ac2c045ac186335.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
