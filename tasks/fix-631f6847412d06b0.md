# fix-631f6847412d06b0 — Isolated subscriber storage — cron job sees a different store than /subscribe handler

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

`E1T1.ts` (line 32), `E3T1.ts` (line 32), and `subscriptions.ts` (line 25) each independently call their own version of `getSubscriberStorage()` / `getSubStore()`. Without `REDIS_URL` set, each creates a separate `MemorySessionStorage` with its own `Map`. Subscribers added via `/subscribe` (E1T1's store) are **invisible** to the daily cron job (E3T1's store) — the cron will iterate its own empty store and never send any morning messages. With Redis, each handler also opens its own Redis connection pool, wasting resources.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-631f6847412d06b0.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-631f6847412d06b0.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-631f6847412d06b0.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
