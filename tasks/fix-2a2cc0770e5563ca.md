# fix-2a2cc0770e5563ca — E1T2 /unsubscribe has no fallback when Redis is unavailable

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

When `REDIS_URL` is not set, `src/store/subscriptions.ts:38` returns `false` immediately in `removeSubscriber()`, making `/unsubscribe` permanently non-functional in any environment without Redis (development, test harness). Contrast with E1T1 (`src/handlers/E1T1.ts:29`) which falls back to `MemorySessionStorage`. The storage abstraction must be consistent across both commands.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-2a2cc0770e5563ca.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-2a2cc0770e5563ca.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-2a2cc0770e5563ca.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
