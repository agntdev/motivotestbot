# fix-1b90c3d03f6539c9 — src/store/subscriptions.ts: duplicate addSubscriber function — compile error

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

`src/store/subscriptions.ts` defines `addSubscriber` twice: at line 27 (correct, using `store.write`) and at line 40 (broken, references undefined `getClient()`, `k()`, `PREFIX`). TypeScript `strict: true` rejects duplicate function implementations in the same scope, so `npm run build` will fail. E3T2 imports `getAllChatIds` from this file, making the entire E3T2 handler unloadable.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-1b90c3d03f6539c9.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-1b90c3d03f6539c9.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-1b90c3d03f6539c9.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
