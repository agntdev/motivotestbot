# fix-99a568f3a85ede2b — Missing Postgres integration — spec requires DATABASE_URL/pg but only Redis/memory used

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

`docs/spec.md` specifies a Postgres `subscribers` table with `DATABASE_URL` env var (lines 62-64, 41-48). The actual implementation uses Redis/memory session storage (`RedisSessionStorage`/`MemorySessionStorage`) with no Postgres dependency listed in `package.json`. The bot stores subscribers under Redis keys prefixed `sub:`, not in a SQL table. This is a fundamental architectural divergence from the spec.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-99a568f3a85ede2b.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-99a568f3a85ede2b.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-99a568f3a85ede2b.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
