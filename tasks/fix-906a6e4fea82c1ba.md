# fix-906a6e4fea82c1ba — src/store/subscriptions.ts: getAllChatIds references undefined symbols — runtime crash

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 MOTV

`getAllChatIds()` at line 47 calls `getClient()` and uses `PREFIX`, neither of which is imported or defined anywhere in the file. Even if the duplicate `addSubscriber` issue were resolved and the file compiled, this function would throw a `ReferenceError` at runtime when E3T2's cron job fires at 09:00 UTC.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-906a6e4fea82c1ba.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-906a6e4fea82c1ba.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.


## Handler module

Implement this feature in its OWN file `src/handlers/fix-906a6e4fea82c1ba.ts` that default-exports a grammY `Composer`. `buildBot()` auto-loads every file in `src/handlers/` at startup, so your handler is wired up automatically. NEVER edit `src/bot.ts` — every feature editing that one shared file makes concurrent PRs conflict. The global error boundary + unknown-command fallback already live in `buildBot()`; do not re-add them.
